...
class Product {
  ...
  showStatus() {
    console.log(
      `${this.name} is ${this.inStock ? "in stock" : "out of stock"}!`,
    );
  }
  applyDiscount(value) {
    // TODO: add validation
...
class Monitor extends Product {
  ...
  show() {
    ...
    console.log(
      `Screen: ${this.resolution.w}x${this.resolution.h}, ${this.type}`,
    );
  }
  ...
  get ppi() {
    return Math.round(
      Math.sqrt(this.resolution.w ** 2 + this.resolution.h ** 2) /
        this.diagonal,
    );
  }
}
